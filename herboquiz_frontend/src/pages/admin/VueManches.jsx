        ...
        <ul className="carte divide-y divide-bord cascade">
          {manches.map((m) => (
            <li key={m.id} className="flex items-center gap-3 px-4 py-3">
              ...
              <label className="flex items-center gap-1.5 text-xs text-texte-faible cursor-pointer"
                     title={t('admin.date_prevue')}>
                <CalendarClock size={13} className="shrink-0" />
                {/* Enregistrement au BLUR, pas a chaque frappe : un datetime-local
                    incomplet (date saisie mais heure pas encore) renvoie une valeur
                    VIDE. En sauvant a chaque onChange, on ecrasait la date par NULL
                    tant que l'heure n'etait pas posee — d'ou « ca ne se sauvegarde
                    pas ». On n'ecrit que quand la valeur a reellement change ; la
                    cle repose le champ sur la valeur serveur apres refetch. */}
                <input type="datetime-local"
                       key={String(m.date_prevue)}
                       defaultValue={m.date_prevue ? String(m.date_prevue).slice(0, 16) : ''}
                       onBlur={(e) => {
                         const saisie = e.target.value || null
                         const actuel = m.date_prevue ? String(m.date_prevue).slice(0, 16) : null
                         if (saisie !== actuel) changerDate.mutate({ manche: m, date: saisie })
                       }}
